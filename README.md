...
exports Welch PSDs in `g^2/Hz` through `save_recovery_psd_csv`. Set
`recovery_psd_spacing = 'one_sixth_octave'` to write 1/6-octave-band averaged
PSDs instead of the linear-Hz Welch grid. The octave-band export uses center
frequencies from `recovery_psd_band_fmin_hz` to `recovery_psd_band_fmax_hz`,
with defaults of `20 Hz` to `2000 Hz`.
...
opts = struct( ...
    ...
    'recovery_psd_spacing', 'one_sixth_octave', ...
    'recovery_psd_band_fmin_hz', 20.0, ...
    'recovery_psd_band_fmax_hz', 2000.0, ...
